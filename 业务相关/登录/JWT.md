...
> 本文参考阮一峰相关文章[https://www.ruanyifeng.com/blog/2018/07/json_web_token-tutorial.html](https://www.ruanyifeng.com/blog/2018/07/json_web_token-tutorial.html)
...
2. 服务器索性不保存 session 数据了，所有数据都保存在客户端，每次请求都发回服务器。JWT 就是这种方案的一个代表。
...
是一个`JSON`对象，描述 `JWT` 的元数据，最后，将此`JSON`对象使用   `Base64URL`算法转成字符串
...
也是一个 JSON 对象，用来存放实际需要传递的数据，JWT 默认是不加密的，任何人都可以读到，所以不要把秘密信息放在这个部分。
